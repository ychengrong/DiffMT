...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None,pos_embed=None):
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1).contiguous())
        #print(self.relative_position_index.view(-1).shape, max(self.relative_position_index.view(-1)), self.relative_position_bias_table.shape)
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1],
            self.window_size[0] * self.window_size[1], -1)  

        #print(self.relative_position_bias_table[self.relative_position_index.view(-1)].shape)
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous() 
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        #print("if min(self.input_resolution) <= self.window_size:"+str(self.input_resolution)+str(self.window_size))
        if min(self.input_resolution) <= self.window_size:
            # if window size is larger than input resolution, we don't partition windows
            self.shift_size = 0
            self.window_size = min(self.input_resolution)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.depth = depth
        # build blocks
        #print("BasicLayer:"+str(input_resolution))
    ...
    def forward(self, x, H, W):
        ...
        for h in h_slices:
            for w in w_slices:
                ...
                img_mask[:, h, w, :] = cnt
                cnt += 1
        #print("+++img_mask={}, self.window_size={}".format(img_mask.shape, self.window_size))
        ...
        for blk in self.blocks:
            #print("***x={}, attn_mask={}".format(x.shape, attn_mask.shape))
            x = blk(x, attn_mask)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        stride1=[patch_size[1]//2,patch_size[2]//2]
        stride2=[patch_size[1]//2,patch_size[2]//2]
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                input_resolution=(
                    ...
                    pretrain_img_size[0] // patch_size[1] // 2 ** i_layer,
                    pretrain_img_size[1] // patch_size[2] // 2 ** i_layer),
        ...
        for i_layer in range(self.num_layers)[::-1]:
            layer = BasicLayer_up(
                ...
                input_resolution=(
                    ...
                    pretrain_img_size[0] // patch_size[1] // 2 ** (len(depths)-i_layer-1),
                    pretrain_img_size[1] // patch_size[2] // 2 ** (len(depths)-i_layer-1)),
                depth=depths[i_layer],
                num_heads=num_heads[i_layer],
                window_size=window_size[i_layer],
                mlp_ratio=mlp_ratio,
...
class final_patch_expanding(nn.Module):
    def __init__(self,dim,num_class,patch_size):
        ...
        # print(dim,num_class,patch_size,patch_size)
        self.up=nn.ConvTranspose2d(dim,num_class,patch_size[1:],patch_size[1:])
