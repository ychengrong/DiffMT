...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            features = 32
            self.hwm = Generic_UNet(self.in_channels - 1, features, 2, 5)#ycr
    ...
    def highway_forward(self,x,hs,*args, **kwargs):
        return self.hwm(x,hs,*args, **kwargs)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        # print(len(self.input_blocks), len(self.output_blocks))
        ...
        for ind, module in enumerate(self.input_blocks):
            ...
            h_0 = h
            h = module(h_0, emb)
            # print("module(h_0, emb) -> h: {}, {}, {}".format(h_0.size(), emb.size(), h.size()))
            hs.append(h)
        ...
        for module in self.output_blocks:
            ...
            h_0 = h
            h = module(h_0, emb)
            # print("module(h_0, emb)s -> h: {}, {}, {}".format(h_0.size(), emb.size(), h.size()))
        h = h.type(x.dtype)
        out = self.out(h)
        return out, cal
...
class ArgmaxLayer(nn.Module):
    ...
    def backward(self, grad_output):
        output = torch.zeros(self.input_shape, dtype=torch.float32, device=self.device)
        indices = torch.argmax(output, dim=1).unsqueeze(1)
        grad_input = torch.zeros_like(output)
        grad_input.scatter_(1, indices, grad_output.unsqueeze(1))
        return grad_input
    
class UNetModelSeg(UNetModel): # subUnelModel-B1
    def __init__(self, in_channels, out_channels, _print = False, _size = 'B1'):
        # 256 2 128 2 2 (16,) 0.0 (1, 1, 2, 2, 4, 4) None False False 1 -1 -1 False False False
        image_size = 256
        in_ch = in_channels#2
        num_channels = 128
        ...
        self._print = _print
        self._size = _size
        # self.downsample = nn.Conv2d(2,1,1,1,0,1)
        # self.upsample = nn.Conv2d(1,2,1,1,0,1)
        # self.argmax = nn.Argmax(dim=1, keepdim=True)#ArgmaxLayer()
        self.final_nonlin=sigmoid_helper
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        hlist= []
        for ind, module in enumerate(self.input_blocks):
            # if ind in [2,5,11,17,4,8,10,14,16]: # Smalling
            #     # print(ind)
            #     hs.append(hs[-1])
            #     continue
            if len(emb.size()) > 2:
            ...
            h_0 = h
            h = module(h_0, emb)
            if self._print: print("{}: input_blocks: module(h_0, emb) -> h: {}, {}, {}".format(ind, h_0.size(), emb.size(), h.size()))
            hs.append(h)
            
        if self._size == 'B':
            uemb = self.highway_forward(c, None, onlyEnc=True) #onlyEnc=True时只取返回encoder输出emb
            h = h + uemb
        if self._size == 'L':
            # uemb,_ = self.highway_forward(c, None) #remove skipconnection 改过Generitic_UNet，否则会有两个输出
            # h = h + uemb
            pass
        if self._size == 'M':
            # print(c.size(), hs[3].size(), hs[6].size(), hs[9].size(), hs[12].size())
            _, cal = self.highway_forward(c, [hs[3],hs[6],hs[9],hs[12]])
            return cal
            
            
        h_0 = h
        h = self.middle_block(h_0, emb)
        if self._print: print("middle_block: h:{} = self.middle_block(h_0:{}, emb:{})".format(h.size(), h_0.size(), emb.size()))
        for module in self.output_blocks:
            h_0 = h
            h = th.cat([h_0, hs.pop()], dim=1)
            if self._print: print("h:{} = th.cat([h_0:{}, hs.pop()], dim=1)".format(h.size(), h_0.size()))
            h_0 = h
            h = module(h_0, emb)
            if self._print: print("output_blocks: module(h_0, emb) -> h: {}, {}, {}".format(h_0.size(), emb.size(), h.size()))
        h = h.type(x.dtype)
        out = self.out(h)
...
class Generic_UNet(SegmentationNetwork): # subUnelModel-S
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 seg_output_use_bias=False, _print=False):
        """
    ...
    def forward(self, x, hs, onlyEnc = False):
        has_hs = len(hs) > 0
        skips = []
        seg_outputs = []
        for d in range(len(self.conv_blocks_context) - 1):
            x_0 = x
            x = self.conv_blocks_context[d](x_0)
            if self._print: 
                print("x({}) = self.conv_blocks_context[d](x_o({}))".format(x.size(), x_0.size()))
            ...
            if not self.convolutional_pooling:
                ...
                x = self.td[d](x_0)
                if self._print: 
                    print("x:{} = self.td[d](x_0:{})".format(x.size(), x_0.size()))
                    # print(self.td[d])
            if hs:
                h = hs.pop(0)
                ddims = h.size(1)
        ...
        x_0 = x
        x = self.conv_blocks_context[-1](x_0)
        if self._print: 
            print("x({}) = self.conv_blocks_context[d](x_o({}))".format(x.size(), x_0.size()))
            # print(self.conv_blocks_context[d])
        emb = conv_nd(2, x.size(1), 512, 1).to(device = x.device)(x)
        if self._print: 
            print("emb({}) = conv_nd(2, x.size(1), 512, 1)(x({}))".format(emb.size(), x.size()))
            # print(conv_nd(2, x.size(1), 512, 1))
        if onlyEnc:  # 只取Encoder部分
            return emb
        ...
        for u in range(len(self.tu)):
            ...
            x = self.tu[u](x_0)
            if self._print: 
                print("x({}) = self.tu[u](x_0({}))".format(x.size(), x_0.size()))
                # print(self.tu[u])
            x_0 = x
            x = th.cat((x_0, skips[-(u + 1)]), dim=1)
            if self._print: 
                print("x({}) = th.cat((x_0({}), skips[-(u + 1)]), dim=1)".format(x.size(), x_0.size()))
            x_0 = x
            x = self.conv_blocks_localization[u](x_0)
            if self._print: 
                print("x({}) = self.conv_blocks_localization[u](x_0({}))".format(x.size(), x_0.size()))
                # print(self.conv_blocks_localization[u])
            if self._deep_supervision:
                #seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x))) #ycr
                seg_outputs.append(self.seg_outputs[u](x))
        if not seg_outputs:
            #seg_outputs.append(self.final_nonlin(self.seg_outputs[0](x))) #ycr
            seg_outputs.append(self.seg_outputs[0](x))

        if not has_hs:
            return seg_outputs[-1] # Segmentation Task
