...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while (
            ...
            try:
                    ...
                    # item = next(data_iter)
                    batch, cond, name = next(data_iter)
            # print(batch.size(), cond.size())
            # 只训前景
            # if not th.sum(cond):
            #     continue
            # for di, dl in zip(item['image'], item['label']):
            self.run_step(batch, cond)
                # self.run_step(th.Tensor(di.permute(0,3,1,2)), th.Tensor(dl.permute(0,3,1,2)))
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            losses = losses1[0]
            sample = losses1[1] # ycr

            loss = (losses["loss"] * weights + losses['loss_cal'] * 10).mean() # ycr
            # loss = losses["dice_loss"].mean()
