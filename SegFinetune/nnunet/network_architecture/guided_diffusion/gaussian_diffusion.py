...
from guided_diffusion.diceloss import DiceLoss
criterion = DiceLoss()
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            # pred_xstart = pred_xstart = process_xstart(model_output)
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # target = res * 0.4 + noise * 0.6 #ycr
            
            # model_output = (cal > 0.5) * (model_output >0.5) * model_output if 2. * (cal*model_output).sum() / (cal+model_output).sum() < 0.75 else model_output
            terms["mse_diff"] = mean_flat((target - model_output) ** 2 ) # eps loss 
            
            terms["loss_cal"] = mean_flat((res - cal) ** 2) # mask loss  #ycr20230218_dicelose #
