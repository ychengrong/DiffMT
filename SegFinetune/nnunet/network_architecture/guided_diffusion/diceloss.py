...
class CEDiceLoss(nn.Module):
    ...
    def __init__(self, alpha=1., beta=1., normalization='sigmoid', ce_weight=None):
        ...
        if ce_weight is None:
            ...
            self.ce = nn.CrossEntropyLoss(weight=torch.from_numpy(np.array(ce_weight)).float().cuda())
    ...
    def forward(self, input, target):
        if target.size(1) > 1:
            ce_target = torch.argmax(target, 1)
        else:
            ce_target = target
        
        return self.alpha * self.ce(input, ce_target) + self.beta * self.dice(input, target)
