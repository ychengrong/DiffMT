...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.max_num_epochs = 250#1000
        self.num_batches_per_epoch = 250 # ycr 250 too much 
        self.num_val_batches_per_epoch = 50 # ycr 50 too much
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            #ycr
            save_epoche = 100
            if (self.epoch + 1) % save_epoche == 0:
                if not os.path.exists(join(self.output_folder, str(self.epoch + 1))):
                    os.mkdir(join(self.output_folder, str(self.epoch + 1)))
                self.save_checkpoint(join(self.output_folder, str(self.epoch + 1), "model_final_checkpoint.model"))
                self.save_checkpoint(join(self.output_folder, str(self.epoch + 1), "model_latest.model"))
                self.save_checkpoint(join(self.output_folder, str(self.epoch + 1), "model_best.model"))
