...
def recursive_find_python_class(folder, trainer_name, current_module):
    ...
    for importer, modname, ispkg in pkgutil.iter_modules(folder):
        # print(current_module + "." + modname, ispkg, trainer_name)
        if not ispkg:
...
def load_model_and_checkpoint_files(folder, folds=None, epoch=".", mixed_precision=None, checkpoint_name="model_best"):
    """
    ...
    fold = str(epoch) #ycr
    trainer = restore_model(join(folds[0], fold, "%s.model.pkl" % checkpoint_name), fp16=mixed_precision)
    ...
    #dirpath = "750"
    all_best_model_files = [join(i, fold, "%s.model" % checkpoint_name) for i in folds]
